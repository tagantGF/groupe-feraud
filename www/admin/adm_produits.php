                        ...
                        <div class="col-sm-9 col-md-9">
						  ...
						  <h1>Produits</h1>
                            ...
                            <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>" name="recgesprod" id="recgesprod">
                                ...
                                <input type="submit" name="valider" id="valider" value="Afficher" class='btn btn-success ml-3'/>
                            </form>
                            
                            <p class="filet-pointille"></p>
                            
                            <?php
                            if(isset($_POST['valider'])){
								if(isset($_POST['famille'])) { $famille = securiseFormulaire($_POST['famille']); }else {$famille = '%';}
								if(isset($_POST['sfamille'])) { $sfamille = securiseFormulaire($_POST['sfamille']); }else {$sfamille = '%';}
                                $lect_produits = "SELECT * FROM illi21_produits WHERE p_famille like '$famille' AND p_sfamille like '$sfamille'  ORDER BY p_ordre ASC;";
                            }else{
                                $lect_produits = "SELECT * FROM illi21_produits ORDER BY p_ordre ASC;";
                            }
                            // echo $lect_produits;
							
                            $res_lect_produits = mysqli_query($lien,$lect_produits) or die ("Erreur lecture produits");
							$row_cnt = mysqli_num_rows($res_lect_produits);
							if($row_cnt > 0)
							{
								while($produits = mysqli_fetch_object($res_lect_produits)){
								?>
								<div class="ligne-admin">
									<div class="libelle">
									<?php 
										if($produits->p_enligne == "1") 
										{
										...
										?>
									<?php echo $produits->p_ordre." - ".$produits->p_nom;?></div>
									<div class="actions">
                                    	<?php if($produits->p_type == 'm')
										{
										?>	
                                        <a href="adm_produits_decli.php?id_prd=<?php echo $produits->id_p; ?>" title="Gestion des déclinaisons"><img src="images/administration/task.png" height="25" /></a>
										<?php
										}
										?>
										<a href="adm_produits_modif.php?id_prd=<?php echo $produits->id_p; ?>" title="Modifier le produit"><img src="images/administration/picto-modifier.png" height="25" /></a>
										<a href="adm_produits_supp.php?id_prd=<?php echo $produits->id_p; ?>" title="Supprimer le produit"><img src="images/administration/picto-supprimer.png" height="25" /></a>
									</div>
								</div>
								<div class="clear"></div>
								<?php	
								...
								echo "<i>Aucun produit trouvé ...</i>";
							}
                            ?>
						  
						  	<div class="legendeadmin">
                                <span style="color:#00b100">&nbsp;&nbsp;&nbsp;<strong><sup>&#9632;</sup></strong> <i style="color:#333333">En ligne</i></span>
                                <span style="color:#d70303"><br />&nbsp;&nbsp;&nbsp;<strong><sup>&#9632;</sup></strong> <i style="color:#333333">Pas en ligne</i></span>
                            </div>
						  <!-- InstanceEndEditable -->                        
                       </div>
                  </div>
