                ...
                <div class="container">
                    <div class="row">
                        ...
                        <div class="col-sm-9 col-md-9">
						  ...
						  <h1>Modifier un code promos</h1>
                                
                          <?php
						  if(isset($_GET['cpr']) && is_numeric($_GET['cpr'])){
							$id_cpr = $_GET['cpr'];
							
							$lect_code_promo = "SELECT * FROM illi21_promo WHERE id_promo = '$id_cpr';";
							$res_lect_code_promo = mysqli_query($lien,$lect_code_promo) or die ("Erreur lecture code promo");
							$code_promo = mysqli_fetch_object($res_lect_code_promo);
							
							
                            if(isset($_POST['modifier']))
							{
                                $libelle 	= securiseFormulaire($_POST['libcode']);
                                $mttc 		= securiseFormulaire($_POST['mttc']);
                                $pourcent	= securiseFormulaire($_POST['pourcent']);
                                $p_actif	= securiseFormulaire($_POST['p_actif']);
								$cadeau		= $code_promo->cadeau;
                                // $datedeb	= securiseFormulaire($_POST['datedeb']);
                                // $datefin	= securiseFormulaire($_POST['datefin']);
                                $datedeb	= "";
                                $datefin	= "";
                                
                                if($mttc > 0 and $pourcent > 0 )
                                {
                                    ?>		
                                    <p>ATTENTION, ce code comporte une valeur en euro et un pourcentage, modification impossible.</p>
                                    <p><a href="adm_promos.php" class="gras">Retour à la gestion des codes</a></p>
                                    <?php	
                                }else{
                                    $maj_code_promo  = "UPDATE illi21_promo SET code_promo = '$libelle', promo_mttc = '$mttc', promo_pourc = '$pourcent', cadeau =  '$cadeau', dtdebut = '$datedeb', dtfin = '$datefin', actif = '$p_actif' WHERE id_promo = '$id_cpr'  limit 1;";
                                    $res_maj_code_promo = mysqli_query($lien,$maj_code_promo) or die ("Erreur modification code promo");
                                    ?>		
                                        <p>Le code a bien été modifié.</p>
                                        <p><a href="adm_promos.php" class="gras">Retour à la gestion des codes</a></p>
                                    <?php	
                                }
                                ...
                                <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>" name="aj_sfam" id="aj_sfam">
                                    <div class="mb-3 mt-4 row">
                                        <label for="libelle" class="col-sm-2 col-form-label">Code : </label>
                                        <div class="col-sm-10">
                                            <input type="text" class="form-control" name="libcode" id="libcode" size="20" maxlength="12" required  value="<?php echo $code_promo->code_promo ;?>" />
                                        </div>
                                    </div>
                                    <?php 
									
									if($code_promo->promo_mttc > 0) 
									{
									?>
                                    <div class="mb-3 mt-12 row">
                                        <label for="libelle" class="col-sm-3 col-form-label">Montant € sur HT : </label>
                                        <div class="col-sm-3">
                                            <input type="text" class="form-control"  name="mttc" id="mttc" size="4" maxlength="3" value="<?php echo $code_promo->promo_mttc ;?>" />
                                        </div>
                                    </div>
                                    <?php 
									} 

                                    if($code_promo->promo_pourc > 0) 
									{
									?>
                                    <div class="mb-3 mt-12 row">
                                        <label for="libelle" class="col-sm-3 col-form-label">Remise % sur HT : </label>
                                        <div class="col-sm-3">
                                            <input type="text" class="form-control"  name="pourcent" id="pourcent" size="4" maxlength="2" value="<?php echo $code_promo->promo_pourc ;?>" />
                                        </div>
                                    </div>
                                    <?php 
									} 
                                    
                                    if($code_promo->cadeau =='1') 
									{
									?>
                                    <div class="mb-3 mt-12 row">
                                        <label for="libelle" class="col-sm-3 col-form-label">Cadeau : </label>
                                        <div class="col-sm-3">
                                            <input type="checkbox" class="form-control"  name="cadeau" id="cadeau" <?php if($code_promo->cadeau == '1') echo "checked"; ?>  value="1" disabled />
                                        </div>
                                    </div>
                                    <?php 
									} 
                                    ...
                                    <div class="mb-3 mt-4 row">
                                        <label for="enligne" class="col-sm-3 col-form-label">Actif : </label>
                                        <div class="col-sm-4">
                                            <select name="p_actif" id="p_actif" class="form-control">
                                                <option value="1" <?php if($code_promo->actif == '1') echo "selected=\"selected\""; ?>>Oui</option>
                                                <option value="0" <?php if($code_promo->actif == '0') echo "selected=\"selected\""; ?>>Non</option>
                                            </select>
                                        </div>
                                     </div>
                                    ...
                                    <div class="mb-3 mt-4 row">
                                        <div class="col-sm-9">
                                        <input type="submit" name="modifier" id="modifier" value="Modifier le code" class="btn btn-success" />
                                        </div>
                                    </div>
                                </form>  
                                <?php
