...
class paymentFormToolbox {
    ...
    /**************** CLASS PROPERTIES **************/
    // Container for Clic&Pay user's account informations
    public $account;
    public $debug;
    ...
    /**
     * Constructor, stores the Clic&Pay user's account informations
     * @param $args array(
     *    $shopID => string, the account site id as provided by Clic&Pay
     *    $certTest => string, certificate, test-version
     *    $certProd => string, certificate, production-version
     *    $ctxMode => string ("TEST" or "PRODUCTION"), the Clic&Pay mode to operate
     *    $platform => string URL
     * )
     */
    ...
    /**
     * getFormFields
     * Utility function, returns all the mandatory data needed by a Clic&Pay form payment
     * as an array
     *
     ...
     * during IPN request from Clic&Pay plat-form
     *
     * @param $fields array, the data received from Clic&Pay, usually the _POST PHP super-global
     *
     * @return array|bool
     */
     ...
     *  for Clic&Pay signature
     *
     * @param $fields array data received from Clic&Pay
     * (typically $_POST)
     *
    ...
    public function getIpn(){
        ...
        if( isset($_POST['vads_trans_status'])){
            ...
            try {
                ...
                $error_msg= '### ERROR - An exception raised during IPN Clic&Pay process:';
