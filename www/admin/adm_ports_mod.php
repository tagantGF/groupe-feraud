                ...
                <div class="container">
                    <div class="row">
                        ...
                        if(isset($_GET['id_p']) && is_numeric($_GET['id_p']))
                        {
                            ...
                            $lect_port = "SELECT * FROM illi21_poids WHERE id_pds = '$id_ports';";
                            $res_lect_port = mysqli_query($lien,$lect_port) or die ("Erreur lecture port");
                            $leport = mysqli_fetch_object($res_lect_port);
                            
                            if(isset($_POST['modifier'])){
                                $montant = securiseFormulaire($_POST['montant']);
                                
                               
                                
                                                
                                $enr_modif_port  = "update  illi21_poids set pds_ct = '$montant'  WHERE id_pds = '$id_ports' limit 1;";
                                $res_enr_modif_port = mysqli_query($lien,$enr_modif_port) or die ("Erreur modification frais de port");
                                ?>		
                                <p>Le montant a bien été modifié.</p>
                                <p><a href="adm_ports.php" class="btn btn-info">Retour à la gestion des frais de port</a></p>
                                <?php	
                            }else{
                                ...
                                <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>" name="mod_sfam" id="mod_sfam">
                                    <div class="mb-3 mt-4 row">
                                        <span class="col-sm-1 col-form-label">De</span>
                                        <label  class="col-sm-2 col-form-label"><?php echo $leport->pds_min ; ?> kg</label>
                                    	<span  class="col-sm-1 col-form-label">à</span>
                                        <label  class="col-sm-2 col-form-label"><?php echo $leport->pds_max ; ?> kg</label>
                                        <label for="libelle"  class="col-sm-3 col-form-label">Montant (€ TTC) : </label>
                                        <div class="col-sm-2">
	                                        <input type="text" name="montant" id="montant" size="10" maxlength="3" class="form-control" value="<?php echo $leport->pds_ct ; ?>" />
                                        </div>
                                    </div>
                                    <div class="mb-3 mt-4 row">
                                        <label>&nbsp;</label>
                                        <input type="submit" name="modifier" id="modifier" value="Modifier le montant" class="btn btn-success" />
                                    </div>
                                </form>  
                                <?php
                            }
                        }else{
                            echo "Port inexistant";
                        }
                        ?>
