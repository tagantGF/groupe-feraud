						  ...
						  <h1>Clients Feraud</h1>
                            ...
                            <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>" name="recgesprod" id="recgesprod">
                                <div class="mb-3 pl-4 row">
                                    ...
                                    <div class="col-sm-6">
                                    	<input type="text" name="nomcli" id="nomcli" maxlength="30" class="form-control" value="<?php if(isset($_POST['nomcli'])) echo $_POST['nomcli']; ?>" />
                                    </div>
                                </div>
                                <div class="mb-3 pl-4 row">              
                                    
                                    <label for="cocli"  class="col-sm-2 col-form-label">Code : </label>
                                    <div class="col-sm-4">
                                    	<input type="text" name="cocli" id="cocli" maxlength="5" class="form-control" value="<?php if(isset($_POST['cocli'])) echo $_POST['cocli']; ?>" />
                                    </div>
                                </div>
                                <input type="submit" name="valider" id="valider" value="Rechercher" class='btn btn-success ml-3'/>
                            </form>
                            ...
                            if(isset($_POST['valider'])){
								if(isset($_POST['nomcli']) and strlen($_POST['nomcli']) > 0) { $nomcli = securiseFormulaire($_POST['nomcli']); }else {$nomcli = '%';}
								if(isset($_POST['cocli'])  and strlen($_POST['cocli']) > 0) { $cocli = securiseFormulaire($_POST['cocli']); }else {$cocli = '%';}
                                $lect_clients = "SELECT * FROM illi21_clientsilli WHERE clicode like '%$cocli%' AND clinom like '%$nomcli%'  ORDER BY clinom ASC;";
                            
								// echo $lect_clients;
								
								$res_lect_clients = mysqli_query($lien,$lect_clients) or die ("Erreur lecture produits");
								$row_cnt = mysqli_num_rows($res_lect_clients);
								if($row_cnt > 0)
								{
									while($lesclients = mysqli_fetch_object($res_lect_clients)){
									?>
									<div class="ligne-admin">
										<div class="libelle"><?php echo $lesclients->clicode." ".$lesclients->clinom;?></div>
										<div class="actions">
											<a href="adm_clients_modif.php?id_cli=<?php echo $lesclients->cliid; ?>"><img src="images/administration/picto-modifier.png" height="25" /></a>
											<a href="adm_clients_supp.php?id_cli=<?php echo $lesclients->cliid; ?>"><img src="images/administration/picto-supprimer.png" height="25" /></a>
										</div>
									</div>
									<div class="clear"></div>
									<?php	
