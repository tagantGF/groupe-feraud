                ...
                <div class="container">
                    <div class="row">
                        ...
                        <div class="col-sm-9 col-md-9">
						  ...
						  <h1>Supprimer une sous famille</h1>
                          
							<?php
                            if(isset($_GET['sfamille']) && is_numeric($_GET['sfamille']))
                            {
                                $id_sfamille = $_GET['sfamille'];
								$lect_sfamille = "SELECT * FROM illi21_familles_sous WHERE id_sous_famille = '$id_sfamille';";
								$res_lect_sfamille = mysqli_query($lien,$lect_sfamille) or die ("Erreur : lecture sous famille");
								$sfamille = mysqli_fetch_object($res_lect_sfamille);
                                
                                if(isset($_POST['supprimer'])){
                                    
                                    
                                    $sup_famille = "DELETE FROM illi21_familles_sous WHERE id_sous_famille = '$id_sfamille' limit 1;";
                                    $res_sup_famille = mysqli_query($lien,$sup_famille) or die ("Erreur suppression sous famille");
                                    ?>		
                                    <p>La sous famille a bien été supprimée.</p>
                                    <p><a href="adm_sous_familles.php" class="gras">Retour à la gestion des slides</a></p>
                                    <?php	
                                }else{
									// Compte le nombre de produit ;
									$lect_prod = "SELECT * FROM illi21_produits WHERE p_sfamille = '$id_sfamille';";
									$res_lect_prod = mysqli_query($lien,$lect_prod) or die ("Erreur lecture produits");
									$row_cnt = mysqli_num_rows($res_lect_prod);
                                    ?>
                                    <p class="alert-danger">Attention à bien détacher les produits rattachés à cette sous famille avant de procéder à sa suppression !</p>

                                    <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>" name="mod_sfam" id="mod_sfam">
                                        <div class="mb-3 mt-4 row">
                                        	<label for="photo2" class="col-sm-3 col-form-label">Sous famille à supprimer : </label>
                                            <div class="col-sm-9 col-form-label">
	                                            <strong><?php echo ($sfamille->sf_libelle); ?></strong>
                                            </div>
                                        </div>
                                        <div class="mb-3 mt-4 row">
                                            <label for="photo2" class="col-sm-6 col-form-label">Nombre de produits rattachés à cette sous famille : </label>
                                            <div class="col-sm-6">
	                                            <strong class="rouge"><?php echo $row_cnt; ?></strong>
                                            </div>
                                        </div>
                                        <div class="mb-3 mt-4 row">
                                            <label>&nbsp;</label>
                                            <input type="submit" name="supprimer" class="btn btn-success" id="supprimer" value="Confirmez la suppression de la sous famille" />
                                        </div>
                                    </form>  
                                    <?php
                                }
                            }else{
                                echo "Sous famille inexistante";
                            }
                            ?>
