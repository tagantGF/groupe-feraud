                    ...
                    <div class="row">
                        ...
                        <div class="col-sm-9 col-md-9">
                          <!-- InstanceBeginEditable name="EditRegion3" -->
							<h1>Modifier une photo d'actualité</h1>
                        ...
                        if(isset($_GET['id_s']) && is_numeric($_GET['id_s']))
                        {
                            ...
                            $lect_photo = "SELECT * FROM illi21_actualites_photos WHERE id_photo = '$id_photo';";
                            $res_lect_photo = mysqli_query($lien,$lect_photo) or die ("Erreur");
                            $laphoto = mysqli_fetch_object($res_lect_photo);
                            
                            if(isset($_POST['modifier'])){
                               
                                $ord_photo = securiseFormulaire($_POST['ordre']);
                                
                                if($_FILES['photo2']['name'] != ''){
                                    $imagenm = envoyerImage(($_FILES['photo2']['name']), $_FILES['photo2']['tmp_name'], "../images/actualites");
                                    if(is_file("../images/actualites/".$laphoto->nm_photo)) unlink("../images/actualites/".$laphoto->nm_photo);
                                }else{
                                    $imagenm = $laphoto->nm_photo;
                                }
                                                
                                $enr_photo		= "update  illi21_actualites_photos set nm_photo = '$imagenm', ordre_photo = '$ord_photo' ";
                                $enr_photo 	   .= "WHERE id_photo = '$id_photo' limit 1;";
								// echo $enr_photo;
                                $res_enr_photo	= mysqli_query($lien,$enr_photo) or die ("Erreur modification photo");
                                ?>		
                                <p>Le photo a bien été modifié.</p>
                                <p><a href="adm_blog_photos.php?_ida=<?php echo $id_actu; ?>" class="gras">Retour à la gestion des photos</a></p>
                                <?php	
                            }else{
                                ...
                                <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>" name="mod_sfam" id="mod_sfam" enctype="multipart/form-data">
                                    ...
                                    <div class="mb-3 mt-4 row">
                                        ...
                                        <div class="col-sm-9">
	                                        <input type="file" name="photo2" id="photo2" size="60"  class="form-control" /> <b><i>Dimension 800px × 600px / 72 dpi JPEG</i></b>
                                        </div>
                                    </div>
                                   <div class="mb-3 mt-4 row">
                                        <label for="photo2"  class="col-sm-3 col-form-label">Image actuelle</label>
                                        <div class="col-sm-9">
	                                        <i><strong><img src="../images/actualites/<?php echo $laphoto->nm_photo; ?>" width="300px" /></strong></i>
                                        </div>
                                    </div> 
                                    <div class="mb-3 mt-4 row">
                                        <label for="libelle"  class="col-sm-3 col-form-label">Ordre : </label>
                                        <div class="col-sm-4">
	                                        <input type="text" name="ordre" id="ordre" size="10" maxlength="3" class="form-control" value="<?php echo $laphoto->ordre_photo ; ?>" />
                                        </div>
                                    </div>
                                    ...
                                    <div class="mb-3 mt-4 row">
                                        <label>&nbsp;</label>
                                        <input type="submit" name="modifier" id="modifier" value="Modifier la photo" class="btn btn-success" />
                                    </div>
                                </form>  
                                <?php
                            }
                        }else{
                            echo "Photo inexistante";
                        }
                        ?>
