						  ...
						  if(isset($_GET['_ida']) && is_numeric($_GET['_ida']))
							{
								...
								if(isset($_POST['modifier'])){
									...
									$titre_actu		= securiseFormulaire($_POST['titre_actu']);
									$soustitre_actu	= securiseFormulaire($_POST['soustitre_actu']);
									$detail_actu	= securiseFormulaire($_POST['detail_actu']);
									$enligne		= securiseFormulaire($_POST['enligne']);
									
									if($_FILES['photo1']['name'] != ''){
										$photo1 = envoyerImage(($_FILES['photo1']['name']), $_FILES['photo1']['tmp_name'], "../images/actualites/alaune");	
										if(is_file("../images/actualites/alaune/".$lactu->photo_alaune)) unlink("../images/actualites/alaune/".$lactu->photo_alaune);
									}else{
										$photo1 = $lactu->photo_alaune;
									}
									
									
									
									$query_modif_actu = "update `illi21_actualites` set";
									$query_modif_actu .= "`titre` = '$titre_actu', `sous_titre` = '$soustitre_actu', `description` = '$detail_actu', `photo_alaune` = '$photo1', `en_ligne` = '$enligne'";
									$query_modif_actu .= "WHERE id_actu = '$id_actu' limit 1;";
									// echo $query_modif_actu;
									...
									?>		
									<p>L'actualité a bien été modifiée.</p>
											
									<p><a href="adm_blog.php" class="btn btn-info">Retour à la gestion du blog</a></p>
									<?php 
								}else{
								?>
								<form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>" name="aj_prod" id="aj_prod" enctype="multipart/form-data">
									...
									<div class="mb-3 mt-4 row">
										<label for="libelle" class="col-sm-2 col-form-label">Détail* : </label>
										<div class="col-sm-4"><textarea name="detail_actu" id="detail_actu"><?php echo $lactu->description; ?></textarea><script>CKEDITOR.replace( 'detail_actu', {height: 500, width:680});</script>
                                        </div>
									</div>
									...
									<div class="mb-3 mt-4 row">
										...
										<div class="col-sm-4">
											<select name="enligne" id="enligne" class="form-control">
												<option value="1" <?php if($lactu->en_ligne == '1') echo 'selected="selected"'; ?> >Oui</option>
												<option value="0" <?php if($lactu->en_ligne == '0') echo 'selected="selected"'; ?> >Non</option>
											</select>
										</div>
									</div>
                                    ...
                                    <?php } ?>
									<div class="mb-3 mt-4 row">
										<label for="libelle" class="col-sm-2 col-form-label">Nouvelle photo a la une : </label>
										<div class="col-sm-6">
											<input type="file" class="form-control" name="photo1" id="photo1" size="60" />
										</div>
										<span class="col-sm-4"><i>Format conseillé 800px x 600px JPEG 72dpi </i></span>
									</div>
									...
									<div class="mb-3 mt-4 row">
										<div class="col-sm-9">
										<input type="submit" name="modifier" id="modifier" value="Modifier l'article" class="btn btn-success" />
										</div>
									</div>
								</form>
								<?php 
