	...
	<div class="pages_internes">
		<div class="container">
			<div class="row">
                ...
                <div class="col-lg-12 bordure" id="panierconfirme">
				<?php
    	        ...
    	        if(creationPanier())
				{
					...
					/* Le contenu du Panier*/
                        ...
                        <div class="row lepanier titrepanier">
							...
							{
							?>
                             <div class="col-lg-1 lepanier_item droit">Remise<br />suppl&eacute;mentaire</div>
                            <?php
							}
							?>
                        ...
                        for ($i=0; $i<$nb_article; $i++)
							{ 
								...
								<div class="row lesligne">
										...
										<?php 
											if($_SESSION['panierfer']['nomproduit'][$i] <> "")
											{
												...
												if($_SESSION['panierfer']['typeprod'][$i] == "s")
												{
													...
													// La photo
													$requet_ = "SELECT id_p , p_photo, p_ref FROM illi21_produits where id_p = '".$_SESSION['panierfer']['idproduit'][$i]."'";
													// echo $requet_;
											...
											if(mysqli_num_rows($res_recpoud))
											{
												...
												if($clientcode <> "")
												{
													...
													if($row_cnt_reduc == 1) 
													{
														...
														?>
														<div class="col-lg-1 droit"><div class="mobile">Remise suppl&eacute;mentaire : </div><?php echo $reduc_pdr_pourcent; ?></div>
														<?php
													}else{
									...
									?>
                                    <div class="col-lg-1 droit"><div class="mobile">P.U. H.T. Remis&eacute; : </div>
										<?php 
											echo number_format(prixclientpoudre($_SESSION['panierfer']['prixunitaire'][$i],$redubat,$reduc_pdr_pourcent), 2, '.', ' ' )." &euro;";	 
											$_SESSION['panierfer']['prixunitaire_rm'][$i] = number_format(prixclientpoudre($_SESSION['panierfer']['prixunitaire'][$i],$redubat,$reduc_pdr_pourcent), 2, '.', '' );
										?>
                                    </div>
                                    <div class="col-lg-1 droit"><div class="mobile">Total HT : </div>
									<?php 
										echo number_format(prixclientpoudre($_SESSION['panierfer']['mttc'][$i],$redubat,$reduc_pdr_pourcent), 2, '.', ' ' )." &euro;"; 
										$totalhtavreduc = $totalhtavreduc + prixclientpoudre($_SESSION['panierfer']['mttc'][$i],$redubat,$reduc_pdr_pourcent); 
										$_SESSION['panierfer']['prixtotal_rm'][$i] = number_format(prixclientpoudre($_SESSION['panierfer']['mttc'][$i],$redubat,$reduc_pdr_pourcent), 2, '.', '' );
										?></div>
							...
							if(isset($_SESSION['panierfer']['code']))
							{
								...
								$valide = $_SESSION['panierfer']['codevalid'];
								...
								if(mysqli_num_rows($res_lect_code))
								{
									...
									if($codelu->promo_pourc > 0)
									{
										...
										if($clientbatifer == '0') 
										{
											...
											$nouvtotal = $_SESSION['panierfer']['montantTotal'] * ($codelu->promo_pourc / 100);
											$nouvtotal = number_format($_SESSION['panierfer']['montantTotal'], 2, '.', '') - number_format($nouvtotal, 2, '.', '');
											
											$reduction = -(number_format($_SESSION['panierfer']['montantTotal'] * ($codelu->promo_pourc / 100), 2, '.', ''));
                                    ...
                                    <div class="row mt-2 lepanierclair">
                                        <div class="col-7"> Code cadeau : <?php echo $_SESSION['panierfer']['lecadeau']; ?></div>
                                    ...
                                    <?php
										...
										?>
                                    	<div class="row mt-2 lepanierclair">
                                       	 	<div class="col-7">&nbsp;Code promo : <?php echo $_SESSION['panierfer']['code']." : ".$_SESSION['panierfer']['lapromo']." "; ?></div>
                                    	    <div class="col-5 droit"><?php  echo $reduction." &euro;"; ?></div>
                                    	</div>    
                                    	<?php
										$_SESSION['panierfer']['mttcreduc'] = $reduction;
									}
								}
							}else{
								$_SESSION['panierfer']['mttcreduc'] = 0 ;
							}
							...
							*/
							?>
                                ...
                                <div class="row lepanierclair">
                                    <div class="col-7">&nbsp;Participation aux frais de port de mati&egrave;res dangereuses et d'emballage </div>
                                    <div class="col-5 droit">
									<?php 
                                ...
                                <div class="row lepanier mb-4">
                                    ...
                                    <?php 
                                            ...
                                            $_SESSION['panierfer']['mttccmd'] = $totalttc;
                                            ...
                                            ?>
                                                <form method="post" action="feraud-mon-panier-methode-paiement-cb-ou-compte-pro" name="cgv" id="cgv">
                                                    <p>
                                                       ...
                                                       <br />
                                                       <input type="submit" name="confirmer-panier" id="confirmer-panier" value="Payer avec mon compte Feraud" class="btn btn-danger"/>
                                                       <input type="submit" name="confirmer-panier" id="confirmer-cb" value="Payer par CB" class="btn btn-danger"/>
                                                    </p>
                                                </form>
