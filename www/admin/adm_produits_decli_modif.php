                ...
                <div class="container">
                    <div class="row">
                        ...
                        <div class="col-sm-9 col-md-9">
						  ...
						  <h1>Modifier une déclinaison</h1>
                           <?php
						   if(isset($_GET['_id_prd']) && is_numeric($_GET['_id_prd']))
							{
								$id_decli = $_GET['_id_prd'];
								
								$lect_decli = "SELECT * FROM illi21_produits_var WHERE id_decli = '$id_decli ' limit 1;";
								$res_lect_decli = mysqli_query($lien,$lect_decli) or die ("Erreur lecture declinaison");
								$ladecli = mysqli_fetch_object($res_lect_decli);
								$id_prod = $ladecli->id_prod;
								if(isset($_POST['ajouter'])){
									...
									$pds_prod	= number_format(securiseFormulaire($_POST['pds_prod']), 3, '.', '');
									$prix_prod	= number_format(securiseFormulaire($_POST['prix_prod']), 3, '.', '');
									
									
									
									
									$query_mod_declic  = "UPDATE `illi21_produits_var` SET `p_nom` = '$nom_prod', `p_ref` = '$ref_prod', `p_prix_ht` = '$prix_prod', `p_poids` = '$pds_prod', `p_enligne` = '$enligne'";
									$query_mod_declic .= "WHERE id_decli = '$id_decli ' limit 1;";
									// echo $query_mod_declic;
									$res_mod_declic	= mysqli_query($lien,$query_mod_declic) or die ("Erreur enregistrement declinaison");
									...
									$query_modif_prod  = "UPDATE `illi21_produits` set  `p_prix_ht` = '$prxproduit' WHERE id_p = '$id_prod ' limit 1;";

									$res_modif_prod	= mysqli_query($lien,$query_modif_prod) or die ("Erreur modification produit");
									
									?>		
									<p>La déclinaison a bien été modifiée.</p>
									
									<p><a href="adm_produits_decli.php?id_prd=<?php echo $id_prod;?>" class="btn btn-info">Retour au produit</a></p>
									<?php 
								}else{
								?>
								<form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>" name="aj_prod" id="aj_prod" enctype="multipart/form-data">
									...
									<div class="mb-3 mt-4 row">
										...
										<div class="col-sm-10">
											<input type="text" class="form-control" name="nom_prod" id="nom_prod" maxlength="128" value="<?php echo $ladecli->p_nom; ?>" required />
										</div>
									 </div>
									<div class="mb-3 mt-4 row">
                                        <label for="libelle" class="col-sm-2 col-form-label">Référence* : </label>
                                        <div class="col-sm-4">
                                            <input type="text" name="ref_prod" id="ref_prod" maxlength="30" required class="form-control"  value="<?php echo $ladecli->p_ref; ?>" />
                                        </div>
                                        <label for="libelle" class="col-sm-2 col-form-label">Poids* : </label>
                                        <div class="col-md-4">
                                            <input type="number" name="pds_prod" id="pds_prod" maxlength="30"  step="0.01"  required class="form-control"   value="<?php echo $ladecli->p_poids; ?>"/>
                                        </div>
                                    </div>
									<div class="mb-3 mt-4 row">
                                        <label for="libelle" class="col-sm-2 col-form-label">Prix H.T.* : </label>
                                        <div class="col-sm-4">
                                            <input type="number" pattern="[0-9]+([\.,][0-9]+)?" inputmode="numeric" step="0.01" name="prix_prod" id="prix_prod" maxlength="30" required class="form-control"  value="<?php echo $ladecli->p_prix_ht; ?>" />
                                        </div>
                                        <label for="enligne" class="col-sm-2 col-form-label">En ligne : </label>
                                        <div class="col-sm-4">
                                            <select name="enligne" id="enligne" class="form-control">
                                                <option value="1" <?php if($ladecli->p_enligne == '1') echo "selected='selected'"; ?>>Oui</option>
                                                <option value="0" <?php if($ladecli->p_enligne == '0') echo "selected='selected'"; ?>>Non</option>
                                            </select>
                                        </div>
									...
									<div class="mb-3 mt-4 row">
											<input type="submit" name="ajouter" id="ajouter" value="Modifier la déclinaison" class="btn btn-success col-sm-3 mr-1" />
											<a href="adm_produits_decli.php?id_prd=<?php echo $id_prod;?>" class="btn btn-danger col-sm-3">Annuler</a>
