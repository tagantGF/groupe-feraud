						  ...
						  <h1>Supprimer un client</h1>
							 ...
							 if(isset($_GET['id_cli']) && is_numeric($_GET['id_cli']))
                        	{
								...
								if(isset($_POST['supprimer']))
								{
									...
									$enr_clients  = "delete from illi21_clientsilli  WHERE cliid = '$id_client' limit 1;";
									$res_enr_clients = mysqli_query($lien,$enr_clients) or die ("Erreur suppression client");
									
									$updateclient = "update `illi21_clients_cpt` set  `clientcode` = '', type_cpt = '0', cli_reduc = '0' where `clientcode` = '$leclient->clicode'";
									// echo $updateclient;
									$res_enr_clients = mysqli_query($lien,$updateclient) or die ("Erreur modification clients");
									...
									<form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>" name="aj_sfam" id="aj_sfam">
										<div class="mb-3 mt-4 row">
											...
											<div class="col-sm-10">
												<input type="text" name="cocli" id="cocli" maxlength="5" required class="form-control"  value="<?php echo $leclient->clicode; ?>" disabled  /> 
											</div>
										</div>
										<div class="mb-3 mt-4 row">
											<label for="nomcli" class="col-sm-2 col-form-label">Nom : </label>
											<div class="col-sm-10">
												<input type="text" name="nomcli" id="nomcli" maxlength="30" class="form-control"  required  value="<?php echo $leclient->clinom; ?>" disabled/> 
											</div>
										</div>
										<div class="mb-3 mt-4 row">
											<label for="cpcli" class="col-sm-2 col-form-label">Code postal : </label>
											<div class="col-sm-10">
												<input type="text" name="cpcli" id="cpcli" maxlength="5" required class="form-control" value="<?php echo $leclient->clicp; ?>"  disabled/> 
											</div>
										</div>
										
										<div class="mb-3 mt-4 row">
											<label>&nbsp;</label>
											<input type="submit" name="supprimer" id="supprimer" value="Supprimer le client" class="btn btn-success" />
										</div>
									</form>  
									<?php
								}
