						  ...
						  <h1>Supprimer une famille</h1>
                            ...
                            if(isset($_GET['famille']) && is_numeric($_GET['famille']))
                            {
                                ...
                                if(isset($_POST['supprimer'])){
                                    ...
                                    $sup_famille = "DELETE FROM illi21_familles WHERE id_famille = '$id_famille' limit 1;";
                                    $res_sup_famille = mysqli_query($lien,$sup_famille) or die ("Erreur suppression famille");
                                    ?>		
                                    <p>La famille a bien été supprimée.</p>
                                    <p><a href="adm_familles.php" class="gras">Retour à la gestion des familles</a></p>
                                    <?php	
                                }else{
									// Compte le nombre de produit ;
									$lect_prod = "SELECT * FROM illi21_produits WHERE p_famille = '$id_famille';";
									$res_lect_prod = mysqli_query($lien,$lect_prod) or die ("Erreur lecture produits");
									$row_cnt = mysqli_num_rows($res_lect_prod);
                                    ?>
                                    <p class="alert-danger">Attention à bien détacher les produits rattachés à cette famille avant de procéder à sa suppression !</p>
                                    <p class="alert-danger">Attention également à bien déplacer les sous familles rattachées à cette famille avant de procéder à sa suppression !</p>
                                    <form method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>" name="mod_sfam" id="mod_sfam">
                                        <div class="mb-3 mt-4 row">
                                        	<label for="photo2" class="col-sm-3 col-form-label">Famille à supprimer : </label>
                                            <div class="col-sm-9 col-form-label">
	                                            <strong><?php echo ($famille->f_libelle); ?></strong>
                                            </div>
                                        </div>
                                        <div class="mb-3 mt-4 row">
                                            <label for="photo2" class="col-sm-6 col-form-label">Nombre de produits rattachés à cette famille : </label>
                                            <div class="col-sm-6">
                                        ...
                                        <div class="mb-3 mt-4 row">
                                            <label>&nbsp;</label>
                                            <input type="submit" name="supprimer" class="btn btn-success" id="supprimer" value="Confirmez la suppression de la famille" />
                                        </div>
                                    </form>  
                                    <?php
                                }
                            }else{
                                echo "Famille inexistant";
                            }
                            ?>
